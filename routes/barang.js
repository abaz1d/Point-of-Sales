...
module.exports = function (db) {
    ...
    //         let vcount = 1
    //         let vsyntax = []
    //         let sql_vcount = `SELECT count(var.id_varian) AS total FROM varian var
    //         INNER JOIN barang bar ON bar.id_barang = var.id_barang`
    //         let vsql = `SELECT var.id_varian,
    //     var.nama_varian,
    //       bar.id_barang,
    //     bar.nama_barang,
    //       var.stok_varian,
    ...
    //             vsql += ' WHERE '
    //             sql_vcount += ' WHERE '
    //             vsearch.push(`%${cari_id}%`)
    //             vsyntax.push(`id_varian ILIKE $${vcount}`)
    //             vcount++
    //         }
    //         if (cari_nama) {
    ...
    //                 sql_vcount += ' WHERE '
    //             }
    //             vsearch.push(`%${cari_nama}%`)
    //             vsyntax.push(` nama_varian ILIKE $${vcount}`)
    //             vcount++
    //         }

    //         if (vsyntax.length > 0) {
    //             vsql += vsyntax.join(' AND ')
    //             vsql += ` ORDER BY id_varian ASC`

    //             sql_vcount += vsyntax.join(' AND ')
    //             sql_vcount += `GROUP BY var.id_varian ORDER BY id_varian ASC`
    //         }


    //         // console.log(sql_vcount, vsearch)
    //         db.query(sql_vcount, vsearch, (err, data) => {
    //             if (err) console.log(err)

    //             // totalPage = Math.ceil(data.rows[0].total / limit)
    //             db.query(vsql, vsearch, (err, data) => {
